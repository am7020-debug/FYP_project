...
% Load the Excel file
filename = 'CostProjection2050Summary.xlsx'; % Specify your Excel file name
...
disp('First few rows of the summary sheet:');
disp(data);

% Extract costs for 'Electroliser' and 'Fuel Cell'
electrolyser_idx = strcmp(data.Technology, 'Electrolyser');
fuel_cell_idx = strcmp(data.Technology, 'Fuel cell');

if any(electrolyser_idx)
    electrolyser_cost = data.CostProjection2050(electrolyser_idx);
else
    electrolyser_cost = 0;
    disp('"Electrolyser" cost not found.');
end

if any(fuel_cell_idx)
    fuel_cell_cost = data.CostProjection2050(fuel_cell_idx);
else
    fuel_cell_cost = 0;
...
other_tech_idx = ~strcmp(data.Technology, 'Electrolyser') & ...
                 ~strcmp(data.Technology, 'Fuel cell');

other_technologies = data.Technology(other_tech_idx);
other_costs = data.CostProjection2050(other_tech_idx);
...
% Adding legend
legend({'Electrolyser', 'Fuel Cell', 'Other Technologies'}, 'Location', 'northwest');
