...
% Load the Excel file
filename = 'tech_cost_specification_1.xlsx';

% List of technologies
technologies = {'Redox flow battery', 'Lithium ion battery', 'Pumped hydro', 'CAES', 'Hydrogen'};

% Variables to be extracted
required_vars = {'Dis_dur', 'C_p_inv', 'C_e_inv', 'N_c', 'C_p_om', 'C_e_om', 'N_op', 'DoD', 'RT', 'Deg_t', 'EoL', 'Life_cyc', 'C_p_eol', 'C_e_eol', 'self_dis'};
...
for i = 1:length(technologies)
    ...
    LCOS_values(i) = L_CAPEX(i) + L_OM(i) + L_CH(i) + L_EOL(i) + L_FUEL(i);
...
results_table = table(technologies', L_CAPEX, L_OM, L_CH, L_EOL, L_FUEL, LCOS_values, ...
    'VariableNames', {'Technology', 'CAPEX', 'O&M', 'Charging', 'End_of_Life', 'L_FUEL', 'LCOS'});

% Save the table to an Excel file
writetable(results_table, 'LCOS_results.xlsx');

% Plotting the stacked bar chart for LCOS breakdown
figure;
bar_data = [L_CAPEX, L_OM, L_CH, L_EOL, L_FUEL];
bar(bar_data, 'stacked');
set(gca, 'XTickLabel', technologies);
legend({'CAPEX', 'O&M', 'Charging', 'End of Life'}, 'Location', 'bestoutside');
ylabel('LCOS ($/MWh)');
